...
function isInedit(ponto, lista) {
  // Verifica se o ponto é inédito na fronteira
  let saida = true;
  lista.map((item) => {
    if (item['id'] == ponto['id']) {
      saida = false;
...
G: é o custo do movimento. Ou seja, a distância de um ponto ao outro.
...
*/
let fronteira = []; // Lista Aberta
let avaliados = []; // Lita Fechada
